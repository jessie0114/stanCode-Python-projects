...
def draw_names(canvas, name_data, lookup_names):
    ...
    # ----- Write your code below this line ----- #
    rank_height = (CANVAS_HEIGHT-GRAPH_MARGIN_SIZE*2)/MAX_RANK
    color_index = 0
    for name in lookup_names:
        ...
        color_index += 1
        for i in range(len(YEARS)):
            x.append(get_x_coordinate(CANVAS_WIDTH, i))
            color = COLORS[color_index % len(COLORS)-1]
            if str(YEARS[i]) in name_data[name]:
                rank = name_data[name][str(YEARS[i])]
                y.append(GRAPH_MARGIN_SIZE + int(rank) * rank_height)
                canvas.create_text(x[i]+TEXT_DX, y[i], text=f'{name} {rank}', anchor=tkinter.SW, fill=color)
            else:
                y.append(CANVAS_HEIGHT-GRAPH_MARGIN_SIZE)
                canvas.create_text(x[i]+TEXT_DX, y[i], text=f'{name}*', anchor=tkinter.SW, fill=color)
            if i > 0:
                canvas.create_line(x[i-1], y[i-1], x[i], y[i], width=LINE_WIDTH, fill=color)
